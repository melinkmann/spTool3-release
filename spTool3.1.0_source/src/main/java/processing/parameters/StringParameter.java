...
public class StringParameter extends AbstractParameter<String> implements Serializable,
    Parameter<String> {
  ...
  public Parameter<String> copyWithoutChildren() {
    Parameter<String> copy = new StringParameter(super.getLabel(),
  ...
  public void trySetValue(Parameter<?> par) {
    if (par instanceof StringParameter) {
      String val = ((StringParameter) par).getValue();
      setValue(val);
    }
  }
  ...
  public FxParameter<String> getObservableInstance() {
    ...
    return new StringFxParameter(this, formatter);
  }

  @Override
  public boolean isEquivalent(Parameter<?> other) {
    boolean equivalent = other instanceof StringParameter;
    // This is cast-safe :-)
    equivalent = equivalent && this.getValue().equals(((StringParameter) other).getValue());
    return equivalent;
  }
