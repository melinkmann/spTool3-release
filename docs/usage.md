...
- `Gating parameters`: This submethod includes options to remove events based on certain peak properties such as 'Number
  ...
  **Note: The peak height-based gating filter is somewhat redundant with the search height
  ...
  The idea is that the search submethod
  ...
  mismatch between statistical model and reality often manifests in an excess of false-positive event detections. In
  response, researchers tend to increase the threshold by choosing extremely high z-values with z > 5 (z as in z·σ,
