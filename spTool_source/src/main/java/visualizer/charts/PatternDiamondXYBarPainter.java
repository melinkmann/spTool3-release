 ...
 */
public class PatternDiamondXYBarPainter implements XYBarPainter, Serializable {
   ...
   */
  public PatternDiamondXYBarPainter() {
    this(0.10, 0.20, 0.80);
  }
   ...
   */
  public PatternDiamondXYBarPainter(double g1, double g2, double g3) {
    this.g1 = g1;
    this.g2 = g2;
    this.g3 = g3;
  }
  ...
  public void paintBar(Graphics2D g2, XYBarRenderer renderer, int row,
    ...
    double diamondSize = Math.max(barWidth * 0.05, 2);
    ...
    // Clip to the bar shape so diamonds don't draw outside
    g2.clip(bar);

    // Get bar bounds for grid extents
    Rectangle2D bounds = bar.getBounds2D();

    // Reduce density by increasing spacing between diamonds
    double spacing = diamondSize * 1.5; // Increase to reduce density
    ...
    for (double x = bounds.getMinX(); x <= bounds.getMaxX(); x += spacing) {
      for (double y = bounds.getMinY(); y <= bounds.getMaxY(); y += spacing) {
        ...
        diamond.moveTo(cx, cy - half);      // top
        diamond.lineTo(cx + half, cy);      // right
        diamond.lineTo(cx, cy + half);      // bottom
        diamond.lineTo(cx - half, cy);      // left
        diamond.closePath();
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof PatternDiamondXYBarPainter)) {
      return false;
    }
    PatternDiamondXYBarPainter that = (PatternDiamondXYBarPainter) obj;
    if (this.g1 != that.g1) {
      return false;
    }
    if (this.g2 != that.g2) {
