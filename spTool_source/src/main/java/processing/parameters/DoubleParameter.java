...
public class DoubleParameter extends AbstractParameter<Double> implements Serializable,
    Parameter<Double> {
  ...
  public Parameter<Double> copyWithoutChildren() {
    Parameter<Double> copy = new DoubleParameter(
        super.getLabel(),
  ...
  public void trySetValue(Parameter<?> par) {
    if (par instanceof DoubleParameter) {
      double val = ((DoubleParameter) par).getValue();
      setValue(val);
    }
  }
  ...
  public FxParameter<Double> getObservableInstance() {
    ...
    return new DoubleFxParameter(this, format, formatter);
  }
  ...
  public boolean isEquivalent(Parameter<?> other) {
    boolean equivalent = other instanceof DoubleParameter;
    // This is cast-safe :-)
    equivalent = equivalent && this.getValue().equals(((DoubleParameter) other).getValue());
    return equivalent;
  }
}
