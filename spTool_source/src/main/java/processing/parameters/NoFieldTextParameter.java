...
public class NoFieldTextParameter extends AbstractParameter<String> implements Serializable,
    Parameter<String> {
  ...
  public Parameter<String> copyWithoutChildren() {
    Parameter<String> copy = new NoFieldTextParameter(super.getLabel(),
  ...
  public void trySetValue(Parameter<?> par) {
    if (par instanceof NoFieldTextParameter) {
      String val = ((NoFieldTextParameter) par).getValue();
      setValue(val);
    }
  }
  ...
  public FxParameter<String> getObservableInstance() {
    ...
    return new NoFieldTextFxParameter(this, formatter);
  }

  @Override
  public boolean isEquivalent(Parameter<?> other) {
    boolean equivalent = other instanceof NoFieldTextParameter;
    // This is cast-safe :-)
    equivalent = equivalent && this.getValue().equals(((NoFieldTextParameter) other).getValue());
    return equivalent;
  }
}