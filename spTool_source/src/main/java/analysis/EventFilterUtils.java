...
public abstract class EventFilterUtils {
   ...
   */
  public static int checkForMultiParticleEvent_derivativeSmooth(double[] tiIntensity, Event event,
                                                                double zIntensity,
    ...
    if (event.getNoOfPoints() > 3) {
      ...
      for (int i = 0; i < values.length - 1; i++) {
        ...
        thr[i] = zIntensity * Math.sqrt(values[i + 1] + 0.5); // add continuity epsilon
      }
      ...
      for (int i = 0; i < sign.size(); i++) {
        ...
        boolean negSegLargeEnough = true; // assume OK as ini state (we start at valid positive sign)
        // segmentSum must be negative (NOT zero, i.e., at least one DP has been added)
        if (sign.get(i).equals("-") && segmentSum < 0) {
          // add continuity epsilon
          negSegLargeEnough = Math.abs(segmentSum) > zSequence * Math.sqrt(segmentLength * bgPerDT + 0.5);
          //if (!negSegLargeEnough) {
          // System.out.println("Negative segment was rejected by filter." );
          // System.out.println(i+ "\t\t segmentSum= "+segmentSum +"\t\t THR= "+zSequence * Math.sqrt
          // (segmentLength * bgPerDT));
          //}
        }
        ...
        if (negSegLargeEnough && sign.get(i).equals(currentSign)) {
           ...
           */
          // add continuity epsilon
          if (segmentSum > zSequence * Math.sqrt(segmentLength * bgPerDT + 0.5)) {
           /*
              - previous condition was based on the peak height, but that is more sample dependent:
   ...
   */
  public static int checkForMultiParticleEvent_rawSmooth(double[] tiIntensity, Event event, double zIntensity,
                                                         double zSequence, int moavPeriod) {
    ...
    if (event.getNoOfPoints() > 3) {
      ...
      // Use the ABSOLUTE signal here, not the net signal (Poisson depends on ABSOLUTE and GROSS!!)
      for (int i = 0; i < smoothValues.length - 1; i++) {
        differences[i] = smoothValues[i + 1] - smoothValues[i];
        thr[i] = zIntensity * Math.sqrt(smoothValues[i + 1] + 0.5); // add continuity epsilon
      }

      List<Double> filteredDifferences = new ArrayList<>();

      for (int i = 0; i < differences.length; i++) {
        // Avoid gaps in data - instead interpolate
        if (Math.abs(differences[i]) > thr[i]) {
          filteredDifferences.add(differences[i]);
        }
      }
      ...
      /*
       Here we skipped the smooth. Why?
       We may smooth away valleys if peaks are very flat. Flat peaks mean small differences,
       i.e., these differences are filtered out. Then, the two flanks get pushed together
       (differences in between are gone) and when if smooth, the edge is gone.
       */
